...
export function POSManagement() {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-8rem)]">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-card rounded-none border p-4 shadow-sm">
        <div className="flex flex-col sm:flex-row gap-3 items-center justify-between mb-4">
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              className="pl-9 h-11 w-full bg-muted/40 border-border/40 rounded-none outline-none focus:ring-2 focus:ring-primary/20 text-sm font-medium transition"
            />
          </div>
        ...
        {loading ? (
          <div className="flex-1 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 animate-pulse">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="bg-muted/10 dark:bg-foreground rounded-none h-40" />
            ))}
          </div>
        ) : error ? (
                ...
                >
                  <div className="aspect-square bg-muted/5 rounded-none overflow-hidden mb-3 relative flex items-center justify-center">
                    {p.image ? (
                      ...
                      <MonitorDot className="h-8 w-8 text-muted-foreground/30" />
                    )}
                    <span className="absolute bottom-2 right-2 text-[10px] font-bold bg-foreground/80 text-white px-2 py-0.5 rounded-md backdrop-blur-sm">
                      {p.barcode ? "SKU" : "POS"}
                    </span>
                  </div>
                  <div>
                    <h3 className="font-bold text-foreground text-xs line-clamp-2 leading-tight min-h-8">
                      {p.name}
                    </h3>
      ...
      <div className="w-full lg:w-[420px] bg-card rounded-none border p-4 flex flex-col shadow-sm">
        ...
        <div className="border border-muted/10 rounded-none p-3 bg-muted/5/50 mb-4">
          {selectedCustomer ? (
            <div className="flex justify-between items-start">
              <div>
                <p className="font-bold text-foreground text-sm">{selectedCustomer.name}</p>
                <p className="text-[11px] text-muted-foreground font-bold mt-1">📞 {selectedCustomer.phone}</p>
                {selectedCustomer.email && <p className="text-[11px] text-muted-foreground font-bold">✉️ {selectedCustomer.email}</p>}
        ...
        <div className="flex-1 overflow-y-auto pr-1 mb-4 space-y-3">
          {orderItems.map((item) => (
            <div key={item.id} className="flex gap-3 pb-3 border-b border-muted/10 items-center justify-between">
              <div className="flex-1 min-w-0">
                <p className="font-bold text-foreground text-xs truncate">{item.name}</p>
                <p className="text-xs text-foreground font-extrabold mt-1">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  ...
                  className="h-8 w-8 rounded-none bg-muted/10 hover:bg-border/40 text-foreground font-extrabold flex items-center justify-center active:scale-95 transition"
                >
                  –
                </button>
                ...
                <button
                  ...
                  className="h-8 w-8 rounded-none bg-foreground hover:bg-foreground text-white font-extrabold flex items-center justify-center active:scale-95 transition"
                >
                  +
                </button>
                <button
          ...
          {orderItems.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center py-12 text-muted-foreground">
              <Coins className="h-10 w-10 text-muted-foreground/30 mb-2" />
        ...
        {/* Total Ledger Block */}
        <div className="pt-4 border-t border-muted/10 space-y-2.5">
          <div className="flex justify-between text-xs font-semibold text-muted/50">
          ...
          <div className="flex gap-3 mt-4">
            <button
              ...
              className="flex-1 h-12 border border-border/40 hover:bg-muted/5 rounded-none text-xs font-bold text-foreground/70 transition active:scale-95"
            >
              Reset
            </button>
            <button
              ...
              disabled={total === 0}
              className="flex-[2] h-12 bg-primary text-primary-foreground hover:bg-primary/95 disabled:opacity-50 rounded-none text-xs font-bold transition shadow-sm active:scale-95"
            >
              Pay & Checkout
            </button>
          </div>
          ...
          <div className="bg-white rounded-none p-6 w-full max-w-lg shadow-2xl animate-in scale-in duration-200">
            ...
            {isNewCustomerOpen ? (
              <form onSubmit={handleCreateCustomer} className="space-y-4">
                <div>
                  <label className="text-xs font-bold text-muted/50 block mb-1">Full Name</label>
                  <input
                    value={newCustName}
                ...
                <div className="flex gap-3 pt-3">
                  <button
                    ...
                    className="flex-1 h-11 border border-border/40 hover:bg-muted/5 rounded-none text-xs font-bold text-foreground/70 transition"
                  >
                    Back to Search
                  </button>
                  <button
                    type="submit"
                    className="flex-1 h-11 bg-primary text-white rounded-none text-xs font-bold hover:bg-primary/95 transition"
                  >
                    Register Customer
                  </button>
                </div>
                ...
                <div className="max-h-60 overflow-y-auto space-y-2 pr-1">
                  {filteredCustomers.map((cust) => (
                    <div
                      ...
                      className="border border-muted/10 hover:border-primary/30 p-3 rounded-none hover:bg-muted/5/50 cursor-pointer flex justify-between items-center transition"
                    >
                      <div>
                        <p className="font-bold text-foreground text-xs">{cust.name}</p>
                        <p className="text-[10px] text-muted-foreground mt-0.5">📞 {cust.phone} | {cust.email}</p>
                      </div>
                      <span className="text-[10px] font-bold text-muted/50 bg-muted/10 px-2 py-0.5 rounded">
                        Pts: {cust.points}
                      </span>
                    </div>
                  ))}
                  {filteredCustomers.length === 0 && (
                    <p className="text-center py-6 text-xs text-muted-foreground font-bold">No customers found</p>
                  )}
                </div>
      ...
      {isPaymentModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-none p-6 w-full max-w-md shadow-2xl animate-in scale-in duration-200">
            ...
            <div className="bg-muted/5 border border-muted/10 p-4 rounded-none mb-4 text-center">
              <span className="text-xs font-bold text-muted/50 block mb-1">TOTAL AMOUNT DUE</span>
            ...
            {paymentMethod === "CASH" && (
              <div className="space-y-4 animate-in fade-in duration-150">
                <div>
                  <label className="text-xs font-bold text-muted/50 block mb-1">CASH RECEIVED</label>
                  <input
                    type="number"
                    value={cashReceived}
                ...
                <div className="flex gap-2">
                  {[2000, 5000, 10000].map((val) => (
                    <button
                      ...
                      className="flex-1 h-9 rounded-none border border-border/40 text-xs font-extrabold text-foreground/70 hover:bg-muted/5 active:scale-95 transition"
                    >
                      Rs. {val}
                    </button>
                  ))}
                ...
                {Number(cashReceived) >= total && (
                  <div className="flex justify-between items-center p-3.5 bg-emerald-50 border border-emerald-100 rounded-none">
            ...
            {paymentMethod === "CREDIT" && (
              <div className="p-4 bg-amber-50 border border-amber-100 rounded-none text-amber-900 text-xs font-bold space-y-1 animate-in fade-in duration-150">
      ...
      {isCompleteOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-none p-8 w-full max-w-sm text-center shadow-2xl animate-in scale-in duration-200">
            ...
            <p className="text-xs text-muted-foreground font-bold mt-1">Invoice Reference: #{completedOrderNo}</p>

            <div className="my-6 border-t border-dashed border-border/40" />
            <p className="text-muted/50 text-xs font-semibold leading-relaxed">
