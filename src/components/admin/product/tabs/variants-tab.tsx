...
export function VariantsTab({ product }: { product: any }) {
  ...
  const openEditVariantModal = (v: any) => {
    ...
    const loc = v.inventory?.[0]?.location || ""
  ...
  const handleAddVariant = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let res
      if (isEditVariant) {
        res = await adminUpdateVariant(session?.user?.id || "", editingVariantId, payload)
      } else {
      ...
      <div className="space-y-4">
        {product.variants?.length === 0 ? (
          ...
          product.variants?.map((v: any) => {
            const totalStock = v.inventory?.reduce((acc: number, inv: any) => acc + inv.quantity, 0) || 0
            return (
              <div key={v.id} className="bg-white dark:bg-background border border-border/40 shadow-[0_20px_40px_-10px_rgba(0,0,0,0.08)] p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
