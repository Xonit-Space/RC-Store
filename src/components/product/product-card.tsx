...
export const ProductCard = memo(function ProductCard({ product, priority = false }: ProductCardProps) {
  ...
  return (
    <div className="relative bg-smoke-dark border border-white/5 hover:border-racing-yellow/30 group flex flex-col h-full overflow-hidden transition-colors">
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <Link 
          ...
          className="relative z-20 font-heading font-bold text-lg text-white uppercase tracking-wide mb-4 hover:text-racing-yellow transition-colors line-clamp-2"
        >
          {product.name}
        </Link>
        
        <div className="mt-auto flex items-center justify-between pt-4">
          <span className="font-mono font-bold text-lg text-white">Rs. {product.price.toLocaleString()}</span>
          <CartIconButton 
            product={product} 
