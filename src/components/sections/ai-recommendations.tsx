...
export async function AIRecommendations() {
  ...
  return (
    <section className="py-20 bg-slate-100 dark:bg-smoke-dark font-sans relative overflow-hidden border-t border-border">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] rounded-full border border-racing-yellow/20 opacity-20 pointer-events-none">
        <div className="absolute top-0 bottom-0 left-1/2 w-[1px] bg-racing-yellow/50 animate-radar-scan origin-bottom" />
        <div className="absolute inset-0 rounded-full border border-racing-yellow/10 scale-75" />
        <div className="absolute inset-0 rounded-full border border-racing-yellow/10 scale-50" />
      </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <Tabs defaultValue="personalized" className="w-full">
          <TabsList className="grid w-full grid-cols-3 max-w-xl mx-auto mb-10 bg-background/50 border border-border p-1 rounded-none">
            ...
            <TabsTrigger value="trending" className="flex items-center justify-center gap-2 text-xs font-heading font-bold py-3 rounded-none data-[state=active]:bg-racing-yellow data-[state=active]:text-foreground dark:text-white data-[state=active]:shadow-[0_0_15px_rgba(255, 204, 0,0.5)] uppercase tracking-wider transition-all">
              <Activity className="h-4 w-4" />
              Trending Tech
            </TabsTrigger>
            <TabsTrigger value="recent" className="flex items-center justify-center gap-2 text-xs font-heading font-bold py-3 rounded-none data-[state=active]:bg-racing-yellow data-[state=active]:text-foreground dark:text-white data-[state=active]:shadow-[0_0_15px_rgba(255, 204, 0,0.5)] uppercase tracking-wider transition-all">
