...
export function CustomerGallery() {
      ...
      <section className="bg-smoke-dark py-24 border-t border-white/5">
        <div className="container mx-auto px-6 md:px-12">
          <div className="flex gap-4 overflow-hidden">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="min-w-[280px] h-[280px] bg-carbon-dark animate-pulse" />
            ))}
          </div>
        </div>
      </section>
  ...
  return (
    <section className="bg-smoke-dark py-24 border-t border-white/5 overflow-hidden">
      <div className="container mx-auto px-6 md:px-12">
        <div className="flex flex-col items-center text-center mb-16">
          <div className="flex items-center gap-2 mb-2">
            <span className="w-2 h-2 rounded-full bg-racing-yellow animate-pulse" />
            <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-racing-yellow">In The Wild</p>
          </div>
          <h2 className="font-heading font-black text-4xl text-white uppercase tracking-wider mb-4">
            Customer Gallery
          </h2>
          <p className="text-muted-foreground flex items-center gap-2">
            Tag us <span className="font-mono text-white">@RCStoreOfficial</span> to be featured
          </p>
        </div>
        ...
        <div className="relative -mx-6 md:-mx-12 px-6 md:px-12 flex gap-4 overflow-x-auto pb-8 snap-x snap-mandatory hide-scrollbar">
          {images.map((img) => (
            <div key={img.id} className="relative group min-w-[280px] sm:min-w-[320px] aspect-square bg-carbon-dark overflow-hidden snap-center border border-white/5 hover:border-racing-yellow/30 transition-colors">
              <img 
                src={img.imageUrl} 
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-carbon-dark via-carbon-dark/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-xs font-bold text-white">{img.authorName}</span>
                  </div>
                </div>
                {img.caption && (
